...
public class notific extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        assert newbar != null;
        newbar.setTitle("Notifications");

        BaseUrl = getResources().getString(R.string.Base_URL);
        RegUrl = BaseUrl+"notific_students.php";//of api

        lv = (ListView) findViewById(R.id.noti_list);//id of list view in xml

        new noti_list().execute();
    }

    class noti_list extends AsyncTask<String,String, JSONObject> {
        ...
        public void onPostExecute(JSONObject jsonObject) {
            try {
                ...
                if(success==1)
                {
                    ...
                    for (int i = 0; i < item_json.length(); i++)
                    {
                        ...
                        String new_group_id=listJsonObj.getString("notification_id");
                        String group_name= listJsonObj.getString("content");
                        String slno= String.valueOf(i);

                        item.setNoti_id(new_group_id);
            ...
            noti_list_adapter adapter=new noti_list_adapter(notific.this,list);
